 ...
 * Hele enheten (denne filen + config.h + alt annet i MAPPEN) kompileres som
 * ÉN oversettingsenhet. Filer utenfor denne mappen er IKKE en del av bygget.
 */
...
// ---------------------------------------------------------------------------
// Opplastning av time-rapport (TLS). Fjernet mye diagnose-kode for korteis skyld;
// de relevante timeoute-ne ligger i config.h.
// ---------------------------------------------------------------------------
...
void loop() {
  ...
  // Doseringssyklus: kjør hver DOSE_INTERVAL_MS.
  if (now - lastDoseMs >= DOSE_INTERVAL_MS) {
    lastDoseMs = now;
    // Planlagt dose per pumpe (ml) — lest fra en fast tabell her for korthet.
